...
class ReadingListPage(BasePage):
    ...
    async def get_all_book_titles(self, list_type: str, threshold: int, user_name: str):
        """
        ...
        # 2. Smart Waiting
        # Wait for the book titles to appear in the DOM to ensure page hydration.
        try:
        ...
        # 3. Data Extraction
        # locator().all_inner_texts() retrieves all matching strings as a list
        titles = await self.page.locator(self.BOOK_TITLES).all_inner_texts()
        
        # 4. Normalization
        # Clean data ensures the E2E script can perform reliable comparisons
    ...
    async def is_book_in_list(self, book_title: str, list_type: str, threshold: int, user_name: str) -> bool:
        """
        Helper method to check if a specific book exists in a specific list.
        """
        titles = await self.get_all_book_titles(list_type, threshold, user_name)
        return book_title.lower().strip() in titles
    
    async def clear_reading_lists(self, user_name: str):
        """
        Clears books directly from the list page by interacting with the active status buttons.
        """
        # Mapping of list type to the text displayed on the active status button
        list_map = {
        ...
        for list_id, btn_text in list_map.items():
            ...
            # Navigate using stable 'load' strategy
            await self.page.goto(target_url, wait_until="load")
            
            # Identify active buttons (usually those with 'activated' state) containing the status text
            ...
            for btn in active_buttons:
                try:
                    # Verify visibility before clicking to avoid Intersecting errors
                    if await btn.is_visible():
                        await btn.click()
                        # Short timeout to allow the site to process the removal (often an AJAX call)
